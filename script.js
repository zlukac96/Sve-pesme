...
window.addEventListener('beforeunload', function (e) {
	...
	let metronomButton = document.querySelector(".metronom-button");
	...
	//// POTREBNO ZA METRONOM



//let izlaz = document.querySelector(".sekunde");
//let btnStart = document.querySelector(".btnStart");
//let btnStop = document.querySelector(".btnStop");
let audioSilence = document.getElementById("audioSilence");
let audioClick1 = document.getElementById("audioClick1");
let audioClick2 = document.getElementById("audioClick2");
let isMetronomWorking = false;
let metronomeTempo = 280;
let metronomeCounter = 0;
let metronomePattern = [2, 0, 0, 2, 0, 0, 0];
let metronom;
if (typeof Worker !== "undefined") {
	metronom = new Worker('metronom.js');
}

function metronomStart(){
	audioSilence.loop = true;
	audioSilence.play();
	metronomeCounter = 0;
	metronom.postMessage(metronomeTempo);
}

function metronomStop(){
	metronom.postMessage("stop");
	audioSilence.pause();
}

//btnStart.addEventListener("click", metronomStart);
//btnStop.addEventListener("click", metronomStop);




metronom.onmessage = function(e) {
	if(metronomeCounter >= metronomePattern.length) metronomeCounter = 0;
//	izlaz.innerText = e.data;

	if(metronomePattern[metronomeCounter] === 1) audioClick1.play();
	if(metronomePattern[metronomeCounter] === 2) audioClick2.play();
...
/// ovde unos za metronom
	...
	function toggleMetronome(){
		...
		if(isMetronomWorking === false){
			isMetronomWorking = true;
			metronomStart();
			metronomButton.children[0].innerText = "Metronom: Uključen";

		} else{
			isMetronomWorking = false;
			metronomStop();
			metronomButton.children[0].innerText = "Metronom: Isključen";
	...
	metronomButton.addEventListener("click", toggleMetronome);
